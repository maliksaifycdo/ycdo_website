  ...
  return (
    <section className="mx-auto max-w-7xl px-8 py-24">
      <div className="mb-16 flex items-end justify-between">
        <div>
          <h2 className="mb-2 text-4xl font-black text-[#1A3A8F]">Impact In Motion</h2>
          <p className="text-lg text-slate-600">Updates from our field offices and surgical camps.</p>
        </div>
        ...
        {news.map((item) => (
          <motion.article key={item.slug} variants={staggerItem} className="group cursor-pointer">
            <Link href={`${ROUTES.NEWS}/${item.slug}`}>
              ...
              <span className="text-sm font-bold uppercase tracking-widest text-[#C0272D]">{item.date}</span>
